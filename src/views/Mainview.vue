<template>
  <div
    :class="{ 'mainsite': true, 'cube-wrapper': !dice, 'backwhite': dice, 'front': direction1.front, 'top': direction1.top, 'left': direction1.left, 'right': direction1.right, 'back': direction1.back, 'buttom': direction1.buttom }">
    <div :class="{ 'mybox': true, 'cube-front': !dice }">
      <router-link to="/weather">
        <span class="material-symbols-outlined"> event_note </span>
        <h2 v-if="ttfalse">오늘의 산책 날씨</h2>
        <h2 v-else>Today_weather</h2>
      </router-link>
    </div>
    <div :class="{ 'mybox': true, 'cube-back': !dice }">
      <router-link to="/baby">
        <span class="material-symbols-outlined"> newspaper </span>
        <h2 v-if="ttfalse">육아수첩</h2>
        <h2 v-else>Baby_Note</h2>
      </router-link>
    </div>
    <div :class="{ 'mybox': true, 'cube-left': !dice }">
      <a href="#none">
        <span class="material-symbols-outlined"> menu_book </span>
        <h2 v-if="ttfalse">이번주에 어디가?</h2>
        <h2 v-else>go where?</h2>
      </a>
    </div>
    <div :class="{ 'mybox': true, 'cube-right': !dice }">
      <router-link to='/gemini'>
        <span class="material-symbols-outlined"> dictionary </span>
        <h2 v-if="ttfalse">제미니 단어장</h2>
    ...
    <div :class="{ 'mybox': true, 'cube-top': !dice }">
      <a href="#none">
        ...
        <h2 v-if="ttfalse">육아정보 유튜브</h2>
        <h2 v-else>Baby Tube</h2>
      </a>
    </div>
    <div :class="{ 'mybox': true, 'cube-bottom': !dice }">
      <a href="#none" @click="this.$router.push('/level')">
        <span class="material-symbols-outlined"> manufacturing </span>
        <h2 v-if="ttfalse">이벤트</h2>
        <h2 v-else>Difficulty change</h2>
      </a>
    </div>
  </div>
</template>
...
export default {
  setup() {
    ...
    return {
      ttfalse, store, dice, rotateclass, rotatebtn, direction1
...
.cube-wrapper.top {
  transform: perspective(2700px) rotateX(-90deg) rotateY(-45deg) matrix3d(cos(var(--applyAngle)), 0, sin(var(--applyAngle) * -1), 0,
      0, 1, 0, 0,
...
.cube-wrapper.front {
  transform: perspective(2700px) rotateX(0deg) rotateY(-45deg) matrix3d(cos(var(--applyAngle)), 0, sin(var(--applyAngle) * -1), 0,
      0, 1, 0, 0,
      sin(var(--applyAngle)), 0, cos(var(--applyAngle)), 0,
      0, 0, 0, 1);
...
.cube-wrapper.back {
  transform: perspective(2700px) rotateX(0deg) rotateY(135deg) matrix3d(cos(var(--applyAngle)), 0, sin(var(--applyAngle) * -1), 0,
      0, 1, 0, 0,
      sin(var(--applyAngle)), 0, cos(var(--applyAngle)), 0,
      0, 0, 0, 1);
